...
import type { Desvio, EtapaProcesso, ProcessoResponse } from "@/types/api";
...
interface ThermalLoadPanelProps {
  /** Qual das duas cadeias este painel abre. `null` esconde o painel inteiro. */
  processo: ProcessoResponse | null;
  titulo: string;
  /** Uma linha dizendo de onde vieram os pontos desta cadeia. */
  legenda: string;
  /**
   * Desvios a listar no fim. Vêm de fora porque não são desta cadeia: são as medições do
   * painel confrontadas com o que os setpoints preveem, e só fazem sentido embaixo da carta
   * medida.
   */
  desvios?: Desvio[];
}
...
/**
 * O gasto térmico de UMA das cadeias, aberto etapa por etapa.
 *
 * A conta é a mesma para as duas — `energia.calcular_balanco` roda igual sobre qualquer
 * cadeia. O que muda é a origem dos pontos: a Carta Atual encadeia o que os instrumentos
 * mediram, a Carta Otimizada a rota mais barata a partir dos dois primeiros. Este painel
 * responde "de onde saiu cada kW"; quem responde "quanto isso custa a mais" é o
 * `ComparacaoPanel`, logo abaixo.
 */
export function ThermalLoadPanel({
  ...
  const { totais, etapas, avisos } = processo;
    ...
    <section className="mt-4 max-w-[1200px] rounded-[10px] border border-gray-200 bg-white p-3 paisagem:hidden">
      ...
      <p className="mb-2 text-[12px] text-slate-500">
        ...
        {processo.custo && (
          <>
            {" "}
            Consumo elétrico equivalente:{" "}
            {formatarNumero(processo.custo.energia_total_kw, 1)} kW —{" "}
            <strong>R$ {formatarNumero(processo.custo.reais_por_hora, 2)}/h</strong>.
          </>
        )}
      </p>
